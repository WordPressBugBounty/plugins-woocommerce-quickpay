...
/**
 * ISO 3166-1 country data used for address and shipping field mapping.
 *
 * Provides lookups by alpha-2, alpha-3, numeric code, ISD (dial) code,
 * country name, and continent. Used when building basket/shipping data
 * for the QuickPay API.
 *
...
class Countries {
	...
	/**
	 * Returns an array of country data indexed by one field and valued by another.
	 *
	 * Both $key_field and $requested_field must be one of: alpha2, alpha3, num, isd, name, continent.
	 * Invalid values fall back to their defaults (null key = sequential index, 'name' value).
	 *
	 * @param string|null $key_field       Field to use as array key (default: 'alpha2'). Pass null for a sequential array.
	 * @param string      $requested_field Field to use as array value (default: 'name').
	 * @return array<string|int, string> Mapped country data.
	 */
	public static function get( $key_field = 'alpha2', $requested_field = 'name' ) {
		$supported_fields = [ 'alpha2', 'alpha3', 'num', 'isd', 'name', 'continent' ];

		// Fall back to sequential index if the requested key field is not supported
		if ( ! in_array( $key_field, $supported_fields, true ) ) {
			$key_field = null;
		}

		// Fall back to country name if the requested value field is not supported
		if ( ! in_array( $requested_field, $supported_fields ) ) {
		...
		foreach ( self::$countries as $k => $country ) {
			if ( $key_field ) {
				$result[ $country[ $key_field ] ] = $country[ $requested_field ];
			} else {
				$result[] = $country[ $requested_field ];
			}
		}
	...
	/**
	 * Returns the ISO 3166-1 alpha-3 code for a given alpha-2 code.
	 *
	 * @param string $alpha2 Two-letter country code, e.g. 'DK'.
	 * @return string|null Three-letter code, or null if not found.
	 */
	public static function alpha3_from_alpha2( $alpha2 ) {
	...
	/**
	 * Returns the ISO 3166-1 alpha-2 code for a given alpha-3 code.
	 *
	 * @param string $alpha3 Three-letter country code, e.g. 'DNK'.
	 * @return string|null Two-letter code, or null if not found.
	 */
	public static function alpha2_from_alpha3( $alpha3 ) {
