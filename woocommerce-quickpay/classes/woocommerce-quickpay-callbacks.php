...
class WC_QuickPay_Callbacks {
	...
	public static function subscription_authorized( $subscription, WC_Order $related_order, $transaction ): void {
		...
		if ( $related_order->get_total() > 0 ) {
			...
			$is_subscription       = WC_QuickPay_Subscription::is_subscription( $related_order );
			$contains_subscription = WC_QuickPay_Order_Utils::contains_subscription( $related_order );
			$is_renewal            = WC_QuickPay_Subscription::is_renewal( $related_order );
